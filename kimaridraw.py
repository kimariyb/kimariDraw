...
def control_process(version_info):
    """
    当使用命令行调用时，执行这段代码
    """
    # 处理命令行参数
    ...
    # 直接双击可执行文件运行方式
        ...
        while True:
            def main_menu():
                ...
                print(f"1. Change the dpi of image. now dpi is: {dpi}")
                print(f"2. Change the format of image. now format is: {image_format}")
                print(f"3. Plot and save the image now!")

            def update_dpi(new_dpi):
                global dpi
                dpi = int(new_dpi)
                print(f"Dpi has been changed to: {dpi}")

            def update_image_format(new_format):
                global image_format
                image_format = new_format
                print(f"Image format has been changed to: {image_format}")
